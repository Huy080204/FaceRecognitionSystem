...
class Face_Recognition:
    ...
    def __init__(self, root):
        ...
        img_bg = PIL.Image.open(r"ImageFaceDetect\bg1.png")
        img_bg = img_bg.resize((1530, 790), PIL.Image.LANCZOS)
        self.photo_img_bg = ImageTk.PhotoImage(img_bg)


        bg_img = Label(self.root, image = self.photo_img_bg)
        bg_img.place(x=0, y=0, width=1530, height=790)

        heading = Label(bg_img, text="Face recognition system", font=("yu gothic ui", 23, "bold"), 
                        bg=BUTTON_FACE, fg=BLUE, bd=0, relief=FLAT)
        heading.place(x=400, y=22, width=650, height=40)
        ...
        #=============Kiem tra xem hom nay co mon hoc can diem danh khong=====
        if not self.subject_array:
            self.lesson_combo['state'] = "disabled"
            self.notify_label['text'] = "Bạn không có môn học nào cần điểm danh hôm nay"
            self.btnOpen['state']= "disabled"

    def setup_left_frame(self):
        self.left_frame = LabelFrame(self.root, bd=2, bg="white", relief=RIDGE, text="Màn hình nhận diện",
                                font=("arial", 12, "bold"))
        self.left_frame.place(x=80, y=70, width=820, height=640)

        self.panel = ttk.Label(self.left_frame,borderwidth=2, relief="groove")
        self.panel.place(x=8, y=60, width=800, height=480)

        #choose lesson to attendance
        self.choose_frame = LabelFrame(self.left_frame, bd=1, bg=BUTTON_FACE, relief=RIDGE,
                                  font=("arial", 11, "bold"))
        self.choose_frame.place(x=8, y=0, width=800, height=50)

        search_label = Label(self.choose_frame, text="Chọn Môn/ID buổi học: ", font=("arial", 12, "bold"), bg=BUTTON_FACE)
        search_label.grid(row=0, column=0, padx=10, pady=10, sticky=W)

        self.selectsub=StringVar()
        self.lesson_combo = ttk.Combobox(self.choose_frame, textvariable=self.selectsub, font=("arial", 12, "italic"), state="readonly",
                                    width=18)
        self.lesson_combo["values"] = self.subject_array
        ...
        choose_type_att=Label(self.choose_frame, text="Chọn loại Điểm Danh: ", font=("arial", 12, "bold"), bg=BUTTON_FACE)
        choose_type_att.grid(row=0, column=2, padx=35, pady=5, sticky=W)

        self.type_attendance=StringVar()
        self.type_combo=ttk.Combobox(self.choose_frame,textvariable=self.type_attendance ,font=("arial", 11, "bold"), state="readonly",
                                    width=10)
        self.type_combo["values"] = ("Vào", "Ra")
        self.type_combo.current(0)
        self.type_combo.grid(row=0, column=3, padx=0, pady=0, sticky=W)


        #notify-attendance
        self.notify_frame = LabelFrame(self.left_frame, bd=1, bg=BUTTON_FACE, relief=RIDGE,
                                       font=("arial", 11, "bold"))
        self.notify_frame.place(x=8, y=550, width=800, height=60)
        self.notify_label = Label(self.notify_frame, text="Thông báo: Vui lòng chọn Môn/ID Buổi học để mở Camera điểm danh !!!", font=("arial", 13, "bold"),
                             bg=BUTTON_FACE,fg="red")
    ...
    def setup_right_frame(self):
        self.right_up_frame = LabelFrame(self.root, bd=2, bg="white", relief=RIDGE, text="Điểm danh thành công",
                                font=("arial", 12, "bold"))
        self.right_up_frame.place(x=1000, y=70, width=420, height=450)
        ...
        self.f_lbl = Label(self.right_up_frame, image=self.photoimg_left,bg="white",borderwidth=2, relief="groove",highlightcolor="darkblue")
        self.f_lbl.place(x=110, y=10, width=190, height=190)

        self.studentID_atten_info=Label(self.right_up_frame, bg="white",
                                font=("arial", 12, "bold"))
        self.studentID_atten_info.place(x=5,y=220,width=410,height=130)

        #IDSV
        self.studentID_label = Label(self.studentID_atten_info, text="ID Sinh Viên:", font=("arial", 13, "bold"), bg="white")
        self.studentID_label.grid(row=0, column=0, padx=10,pady=10, sticky=W)

        self.studentID_atten_label = Label(self.studentID_atten_info, text="", font=("arial", 13, "bold"),
                                bg="white")
        self.studentID_atten_label.grid(row=0, column=1, padx=10, pady=10, sticky=W)


        #TenSV
        self.studentname_label = Label(self.studentID_atten_info, text="Tên Sinh Viên:", font=("arial", 13, "bold"),
                                     bg="white")
        self.studentname_label.grid(row=1, column=0, padx=10, pady=10, sticky=W)

        self.studentname_atten_label = Label(self.studentID_atten_info, text="", font=("arial", 13, "bold"),
                                           bg="white")
        ...
        #======================Class-info==============================
        self.right_down_frame = LabelFrame(self.root, bd=2, bg="white", relief=RIDGE, text="Thông tin buổi học",
                                      font=("arial", 12, "bold"))
        self.right_down_frame.place(x=1000, y=540, width=420, height=220)

        #Class
        self.className_label = Label(self.right_down_frame, text="Lớp tín chỉ:",
                                     font=("arial", 12, "bold"), bg="white")
        self.className_label.grid(row=0, column=0, padx=10, pady=20, sticky=W)

        self.className_atten_label = Label(self.right_down_frame, text="", font=("arial", 12, "bold"),
                                           bg="white",fg="red2")
        self.className_atten_label.grid(row=0, column=1, padx=10, pady=20, sticky=W)

        # subject/lesson
        self.subject_lesson_label = Label(self.right_down_frame, text="Tên môn học/ID Buổi học:",
                                       font=("arial", 12, "bold"),
                                       bg="white")
        self.subject_lesson_label.grid(row=1, column=0, padx=10, pady=20, sticky=W)

        self.subject_lesson_atten_label = Label(self.right_down_frame, text="", font=("arial", 12, "bold"),
                                             bg="white",fg="red2")
        self.subject_lesson_atten_label.grid(row=1, column=1, padx=10, pady=20, sticky=W)

        # Time
        self.classtime_label = Label(self.right_down_frame, text="Thời gian:",
                                        font=("arial", 12, "bold"),
                                        bg="white")
        self.classtime_label.grid(row=2, column=0, padx=10, pady=20, sticky=W)

        self.classtime_atten_label = Label(self.right_down_frame, text="",
                                              font=("arial", 12, "bold"),
                                              bg="white",fg="red2")
    ...
    def is_clicked(self):
        ...
        self.notify_label['text'] = "Vui lòng chọn ID Buổi học/Tên môn học để điểm danh"
    ...
    def callbackFunc(self, event):
        ...
        if self.selectsub.get() == "":
            ...
            self.db.connect()
            self.db.my_cursor.execute("select Time_start,Time_end,Class from lesson,subject where `subject`.Subject_id=lesson.Subject_id and Lesson_id=%s ",
                              (self.lessonid,))
            getInfo=self.db.my_cursor.fetchone()
    ...
    def mark_attendance(self, i, r, n, d, face_cropped):
        ...
        while True:# khi camera mở lên không có lỗi
            ...
            chkStudent = self.db.my_cursor.fetchall()
            chkarray = []
            ...
            if(i not in chkarray):
                ...
                try:
                    self.db.connect()
                    self.db.my_cursor.execute("select Date, Lesson_id from attendance where Student_id=" + str(i))

                    idn = self.db.my_cursor.fetchall()
                    a = [] #mảng ngày
                    b = [] #mảng lesson_id
                    ...
                    if(self.type_attendance.get()=="Vào"):
                        if((d1 not in a)) or ((self.lessonid not in b)):
                            self.db.my_cursor.execute("insert into attendance values(%s,%s,%s,%s,%s,%s,%s,%s,%s)", (
                                masp,
                                str(i),
                                n,
                                d,
                            ...
                            self.f_lbl = Label(self.right_up_frame, image=self.photoimg_left, bg="white", borderwidth=1,
                                                relief="groove")
                            self.f_lbl.place(x=110, y=10, width=190, height=190)
                            ...
                            self.studentname_atten_label = Label(self.studentID_atten_info, text=n,
                                                                    font=("arial", 13, "bold"), relief="sunken",
                                                                    width=18,
                            ...
                            self.db.my_cursor.execute("Select Time_in from attendance where Student_id=%s and Lesson_id=%s ",(str(i),(self.lessonid),))
                            ckTime_in = self.db.my_cursor.fetchone()
                            time_in = ckTime_in[0]
                            # print(time_in)

                            # -======Timestart========

                            self.db.my_cursor.execute("Select Time_start from lesson where Lesson_id=%s ",(self.lessonid,))
                            ckStart_in = self.db.my_cursor.fetchone()
                            ...
                            if(time_in<time_start):
                                ...
                                if (b < a < c):
                                    ...
                                    # print(stt)
                                    self.db.my_cursor.execute("update  attendance set AttendanceStatus=%s where Student_id=%s and Lesson_id=%s",
                        ...
                        self.db.connect()
                        self.db.my_cursor.execute("select IdAuttendance from attendance")
                        idatt = self.db.my_cursor.fetchall()
                        att=[]
                        for ida in idatt:
                        ...
                        if(masp not in att):
                            if ((d1 not in a)) or ((self.lessonid not in b)):
                                ...
                                self.f_lbl = Label(self.right_up_frame, image=self.photoimg_left, bg="white",
                                                    borderwidth=1,
                                                    relief="groove")
                                self.f_lbl.place(x=110, y=10, width=190, height=190)
                                ...
                                self.db.my_cursor.execute(
                                    ...
                                    (str(i), (self.lessonid),))
                                ckTime_out = self.db.my_cursor.fetchone()
                                time_out = ckTime_out[0]
                                # print(time_out)

                                # -======Timeend========

                                self.db.my_cursor.execute("Select Time_end from lesson where Lesson_id=%s ",
                                                    (self.lessonid,))
                                ckend_in = self.db.my_cursor.fetchone()
                                time_end = ckend_in[0]
                                # print(time_start)
                            ...
                            self.db.my_cursor.execute("select Time_out from attendance where IdAuttendance=%s",(masp,))
                            timeout_check = self.db.my_cursor.fetchone()
    ...
    def face_recog(self):
        ...
        def draw_boundray(img, classifier, scaleFactor, minNeighbors, color, text, clf):
            ...
            for(x, y, w, h) in features:
                ...
                self.db.my_cursor.execute("select Name from student where Student_id=" + str(id))
                n=self.db.my_cursor.fetchone()

                n= "+".join(n)

                self.db.my_cursor.execute("select Roll from student where Student_id=" + str(id))
                r = self.db.my_cursor.fetchone()
                r = "+".join(r)

                self.db.my_cursor.execute("select Class from student where Student_id=" + str(id))
                d = self.db.my_cursor.fetchone()
                d = "+".join(d)

                self.db.my_cursor.execute("select Student_id from student where Student_id=" + str(id))
                i = self.db.my_cursor.fetchone()
                i = i[0]
                self.db.close()
