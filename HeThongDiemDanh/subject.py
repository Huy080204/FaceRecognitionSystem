...
class Subject:
    def __init__(self,root):
        ...
        #====title=========
        self.heading = Label(self.root, text="Subject Information Management", font=("yu gothic ui", 28, "bold"), bg="white", fg="black",
                             bd=5, relief=FLAT)
    ...
    def getNextid(self):
        ...
        lastid = self.db.my_cursor.fetchone()
        if (lastid == None):
    ...
    def add_data(self):
        self.db.connect()
        #=========check subject============
        self.db.my_cursor.execute("select Subject_name from `subject` where Class=%s",(self.var_subclass.get(),))
        ckSubject = self.db.my_cursor.fetchall()
        arraySub=[]
        ...
        #========check class================
        self.db.my_cursor.execute("select Class from `class` ")
        ckclass = self.db.my_cursor.fetchall()
        arrayClass=[]
        for chc in ckclass:
            print(chc[0])
            arrayClass.append(str(chc[0]))
        #============checkid=================
        self.db.my_cursor.execute("select Subject_id from `subject` ")
        cksubject = self.db.my_cursor.fetchall()
        arraySubject = []
        ...
        if self.var_subid.get()=="Select" or self.var_subclass.get()=="" or self.var_subname.get()=="":
            ...
            try:
                self.db.connect()
                self.db.my_cursor.execute("insert into subject values(%s,%s,%s)",(
                    self.var_subid.get(),
    ...
    def fetch_data(self):
            ...
            # mydata.clear()
            self.db.connect()
            self.db.my_cursor.execute("Select * from subject")
        ...
        if self.var_subid.get()=="Select" or self.var_subname.get()=="" or self.var_subclass.get()=="":
            ...
            try:
                ...
                if Update>0:
                    ...
                    self.db.my_cursor = conn.cursor()
                    self.db.my_cursor.execute("update subject set Subject_name=%s,Class=%s"
                                      " where Subject_id=%s",(
                                            self.var_subname.get(),
                                            self.var_subclass.get(),
                                            self.var_subid.get(),
    ...
    def search_data(self):
        if self.var_com_search.get()=="" or self.var_search.get()=="":
            ...
            try:
                conn = mysql.connector.connect(host='localhost', user='root', password='',
                                               database='face_recognizer', port='3306')
                self.db.my_cursor = conn.cursor()#"ID Điểm Danh", "Ngày", "ID Sinh Viên"
                ...
                self.db.my_cursor.execute("select * from subject where "+str(self.var_com_search.get())+" Like '%"+str(self.var_search.get())+"%'")
                data=self.db.my_cursor.fetchall()
                if(len(data)!=0):
                    self.AttendanceReportTable.delete(*self.AttendanceReportTable.get_children())
                    for i in data:
    ...
    def callback(self):
        self.db.connect()
        self.db.my_cursor.execute("select Teacher_id from `teacher` ")
        ckteacher = self.db.my_cursor.fetchall()
        arrayTeacher = []
        ...
        if(self.var_teachersub.get() not in arrayTeacher):
            ...
            self.db.connect()
            self.db.my_cursor.execute("select Name from `teacher` where Teacher_id=%s", (self.var_teachersub.get(),))
            ckteacher = self.db.my_cursor.fetchone()
    ...
    def callsubtc(self):
        ...
        if(self.var_subsub.get() not in arrayTeacher):
            ...
            self.db.connect()
            self.db.my_cursor.execute("select Subject_name from `subject` where Subject_id=%s", (self.var_subsub.get(),))
            ckteacher = self.db.my_cursor.fetchone()
    ...
    def add_Tcdata(self):
        self.db.connect()
        # =========check subject============
        self.db.my_cursor.execute("select Subject_id from teacher_has_subject where Teacher_id=%s", (self.var_teachersub.get(),))
        ckSubject = self.db.my_cursor.fetchall()
        arraySub = []
        for chk in ckSubject:
            arraySub.append(str(chk[0]))
        ...
        self.db.my_cursor.execute("select COUNT(Teacher_id) from `teacher_has_subject` where Subject_id=%s",(self.var_subsub.get(),) )
        ckTeacher_has1sub = self.db.my_cursor.fetchone()
        ...
        if self.var_teachersub.get()=="" or self.var_subsub.get()=="" :
            ...
            try:
                self.db.connect()
                self.db.my_cursor.execute("insert into teacher_has_subject values(%s,%s)",(
    ...
    def fetch_Tcdata(self):
            ...
            # mydata.clear()
            self.db.connect()
            self.db.my_cursor.execute("Select * from teacher_has_subject")
            data = self.db.my_cursor.fetchall()
            if len(data) != 0:
    ...
    def update_Tcdata(self):
        self.db.connect()
        # =========check subject============
        self.db.my_cursor.execute("select Subject_id from teacher_has_subject where Teacher_id=%s",
                          (self.var_teachersub.get(),))
        ckSubject = self.db.my_cursor.fetchall()
        arraySub = []
        for chk in ckSubject:
        ...
        if self.var_teachersub == "" or self.var_subsub.get() == "":
            ...
            try:
                ...
                if Update>0:
                    conn=mysql.connector.connect(host='localhost', user='root', password='', database='face_recognizer', port='3306')
                    self.db.my_cursor = conn.cursor()
                    self.db.my_cursor.execute("UPDATE `teacher_has_subject` SET `Teacher_id` = %s, `Subject_id` = %s WHERE "
    ...
    def search_Tcdata(self):
        if self.var_com_searchtc.get()=="" or self.var_searchtc.get()=="":
            ...
            try:
                ...
                self.db.my_cursor.execute("select * from teacher_has_subject where "+str(self.var_com_searchtc.get())+" Like '%"+str(self.var_searchtc.get())+"%'")
                data=self.db.my_cursor.fetchall()
                if(len(data)!=0):
                    self.TeacherTable.delete(*self.TeacherTable.get_children())
                    for i in data:
    ...
    def callstudent(self):
        self.db.connect()
        self.db.my_cursor.execute("select Student_id from `student` ")
        ckStudent = self.db.my_cursor.fetchall()
        arrayStudent = []
        ...
        if(self.var_studentsub.get() not in arrayStudent):
            ...
            self.db.connect()
            self.db.my_cursor.execute("select Name from `student` where Student_id=%s", (self.var_studentsub.get(),))
            ckStudent = self.db.my_cursor.fetchone()
    ...
    def callstSub(self):
        ...
        if(self.var_subsubst.get() not in arraySubject):
            ...
            self.db.connect()
            self.db.my_cursor.execute("select Subject_name from `subject` where Subject_id=%s", (self.var_subsubst.get(),))
            cksubject = self.db.my_cursor.fetchone()
    ...
    def add_Stddata(self):
        self.db.connect()
        # =========check subject============
        self.db.my_cursor.execute("select Subject_id from student_has_subject where Student_id=%s",
        ...
        if self.var_studentsub == "" or self.var_subsubst.get() == "":
            ...
            try:
                conn = mysql.connector.connect(host='localhost', user='root', password='',
                                                   database='face_recognizer', port='3306')
                self.db.my_cursor = conn.cursor()
                self.db.my_cursor.execute("insert into student_has_subject values(%s,%s)", (
                        self.var_studentsub.get(),
    ...
    def fetch_Stddata(self):
            ...
            # mydata.clear()
            self.db.connect()
            self.db.my_cursor.execute("Select * from student_has_subject")
            data = self.db.my_cursor.fetchall()
            if len(data) != 0:
            ...
            try:
                ...
                if Update > 0:
                    conn = mysql.connector.connect(host='localhost', user='root', password='',
                                                       database='face_recognizer', port='3306')
                    self.db.my_cursor = conn.cursor()
                    self.db.my_cursor.execute("UPDATE `student_has_subject` SET `Student_id` = %s, `Subject_id` = %s WHERE "
    ...
    def search_Stddata(self):
            if self.var_com_searchstd.get() == "" or self.var_searchstd.get() == "":
                ...
                try:
                    ...
                    self.db.my_cursor.execute("select * from student_has_subject where " + str(
